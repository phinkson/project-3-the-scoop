...
function updateComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const requestComment = request.body && request.body.commant;
  const response = {};
  ...
  if (!requestComment) {
    ...
    const savedComment = database.commants[id];
    if (!savedComment || !savedComment.body) {
      response.status = 404;
    }
    else if (savedComment.username === requestComment.username &&
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    // remove all references to its ID from corresponding user and article models
